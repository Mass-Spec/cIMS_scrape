...
def main():
    ...
    if len(sys.argv) >> 2:
        ...
        base_dir = (r'/Users/ashtynasay')

    # Find xml files and store in list

    path_to_xml = find_xml_files(base_dir)
    big_list = []

    #Extracts all usefull information

    for i in range(len(path_to_xml)):

        try:

            extern_info = ((extern_extract(xml_path_to_extern(path_to_xml[i]))))
            header_info = header_extract(xml_path_to_header(path_to_xml[i]))
            xml_info = [xml_ej_height(path_to_xml[i])]
            path = [path_to_xml[i]]
            filename = [(os.path.basename(path_to_xml[i]))]


            # Stores info in a list of lists

            big_list.append(filename + extern_info + xml_info + header_info + path)

        except:
            print("Error at" + str(path_to_xml[i]))
    ...
    cwd = os.getcwd()
    path = cwd + "/cIMS_CSV" + ".csv"
    master_df.to_csv(path, sep = '\t')
...
# Functions that takes a path to an xml file as input and outputs the path to the header file and extern file

def xml_path_to_extern(path):
    return path.replace(os.path.basename(path), '_extern.inf')

def xml_path_to_header(path):
    return path.replace(os.path.basename(path), '_header.txt')
